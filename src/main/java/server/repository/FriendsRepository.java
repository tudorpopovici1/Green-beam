...
public interface FriendsRepository extends JpaRepository<Friends, Long> {
    ...
    @Transactional
    @Modifying
    @Query(value = "UPDATE server.model.Friends f SET f.relationshipType = ?1 "
            + "WHERE f.relatingUserId = ?2 AND f.relatedUserId = ?3")
    void updateRelationshipType(String relationshipType, Long relatingUserId, Long relatedUserId);

    @Transactional
    @Modifying
    @Query(value = "DELETE FROM server.model.Friends f "
            + "WHERE f.relatingUserId = ?1 AND f.relatedUserId = ?2")
